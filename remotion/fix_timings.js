...
['ScenesIntro.tsx', 'ScenesCore.tsx', 'ScenesExecution.tsx'].forEach(file => {
  ...
  SCENES.filter(s => s.file === file).forEach(scene => {
    ...
    content = content.replace(regex, (match) => {
      ...
      updatedMatch = updatedMatch.replace(/L\((\d+)\)/g, (full, gStr) => {
        const oldG = parseInt(gStr, 10);
        const rel = oldG - scene.oldStart;
        const newRel = Math.floor(rel * (scene.newLen / scene.oldLen));
        return `${newRel}`;
      });
      
      // Also need to catch cases where L was used with expressions like L(2290 + 30) -> which matched as L(2290 + 30)?
      // Actually my regex L\((\d+)\) only catches digits. Let's see if there are any math expressions.
      return updatedMatch;
    });
  });
