...
OpenAI price-cut video (oop.wav).
...
captions = json.loads((ROOT / "src" / "oop-captions.json").read_text())
...
TOKEN_MERGES = [
    (["o", "op"], "OOP"),
    (["polymorph", "ism"], "polymorphism"),
    (["poly", "morph", "ism"], "polymorphism"),
    (["poly", "mer", "is"], "Polymorphism"),
    (["encaps", "ulation"], "encapsulation"),
    (["abstract", "ion"], "abstraction"),
    (["ret", "ry"], "retry"),
    (["not", "ifiers"], "notifiers"),
    (["not", "ifier"], "notifier"),
    (["test", "able"], "testable"),
    (["weld", "ed"], "welded"),
    (["scar", "ier"], "scarier"),
    (["ind", "irection"], "indirection"),
    (["in", "jection"], "Injection"),
    (["over", "sell"], "oversell"),
    (["inher", "its"], "inherits"),
    (["past", "ed"], "pasted"),
...
PHRASE_FIXES = [
    (["miss", "that", "'s", "test"], ["miss.", "That's", "a", "test."]),
    (["that", "'s", "test"], ["That's", "a", "test."]),
    (["ret", "ry"], ["retry"]),
    # --- contradicts the code/graphic on screen ---
    (["here's", "a", "doubt", "class"], ["here's", "a", "Dog", "class"]),
    (["this", "alone", "aren't", "oop"], ["Classes", "alone", "aren't", "OOP"]),
    (["three", "specified", "classes"], ["three", "specific", "classes"]),
    (["new", "else", "-", "if", "branch", "insert", "the", "code"],
     ["new", "elif", "branch,", "inside", "the", "code"]),
    (["add", "another", "else-if"], ["add", "another", "elif"]),
    (["it's", "the", "open-closed", "principle"], ["It's", "the", "open-closed", "principle"]),
    # --- clear mishearings ---
    (["most", "oop", "tutorial", "gives", "you", "definition"],
     ["Most", "OOP", "tutorials", "give", "you", "definitions"]),
    (["and", "i", "want", "to", "be", "clear", "there's", "fine"],
     ["And", "I", "want", "to", "be", "clear,", "this", "is", "fine"]),
    (["because", "when", "features", "get", "added", "for", "example", "email"],
     ["Because", "features", "get", "added.", "Email"]),
    (["there's", "34", "lines", "now"], ["That's", "34", "lines", "now"]),
    (["the", "email", "logic", "leaves", "with", "email"], ["The", "email", "logic", "lives", "with", "email"]),
    (["the", "sms", "logic", "leaves", "with", "sms"], ["the", "SMS", "logic", "lives", "with", "SMS"]),
    (["and", "each", "real", "notifier", "keeps"], ["Then", "each", "real", "notifier", "keeps"]),
    (["look", "up", "at", "the", "notifier"], ["look", "up", "the", "notifier"]),
    (["and", "here's", "what", "buys", "you"], ["And", "here's", "what", "that", "buys", "you"]),
    (["that", "record", "message", "instead"], ["that", "records", "messages", "instead"]),
    (["you", "walk", "toward"], ["you", "work", "toward"]),
    (["the", "number", "of", "edit", "isn't"], ["the", "number", "of", "edits", "isn't"]),
    (["in", "the", "first", "three", "version"], ["In", "the", "first", "three", "versions"]),
    (["that's", "a", "code", "that", "already", "worked"], ["That's", "code", "that", "already", "worked"]),
    (["that", "other", "thing", "also", "depend", "on"], ["that", "other", "things", "already", "depend", "on"]),
    (["you", "didn't", "open", "it", "all"], ["you", "didn't", "open", "it", "at", "all"]),
    (["you", "added", "a", "new", "code", "instead", "of", "changing", "the", "old", "code"],
     ["You", "added", "new", "code", "instead", "of", "changing", "old", "code"]),
    (["when", "adding", "a", "feature", "that", "start"], ["when", "adding", "a", "feature", "starts"]),
    (["all", "four", "versions", "are", "in", "description"],
     ["All", "four", "versions", "are", "in", "the", "description"]),
    (["if", "you", "want", "to", "follow", "up", "where"], ["If", "you", "want", "a", "follow-up", "where"]),
    (["with", "the", "payment", "system"], ["with", "a", "payment", "system"]),
    (["close", "to", "modification"], ["closed", "to", "modification"]),
]
...
out_path = ROOT / "src" / "oop-subtitles.json"
