#!/usr/bin/env python3
"""
Same grouping logic as build-crdt-subtitles.py / build-hashtable-subtitles.py,
pointed at the GPT video's captions -- groups raw whisper word-level
captions into stable subtitle "cues" (whole phrases held for their full
spoken duration, per-word karaoke timing) instead of a flickery per-token
window.
"""
import json
...
captions = json.loads((ROOT / "src" / "gpt-captions.json").read_text())
...
out_path = ROOT / "src" / "gpt-subtitles.json"
