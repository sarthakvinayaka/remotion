...
def predict_next_id(context, emit_candidates=False, run=None):
...
def generate(seed_words, length=10, trace=False, run=None):
    context_ids = [word_to_id[w] for w in seed_words]
    result_ids = list(context_ids)
    if trace:
        emit({"type": "generate_start", "run": run, "context_ids": list(result_ids)})
    for _ in range(length):
        context = tuple(result_ids[-CONTEXT_SIZE:])
        next_id = predict_next_id(context, emit_candidates=trace, run=run)
        if next_id is None:
            break
        result_ids.append(next_id)
        if trace:
            emit({"type": "append", "run": run, "context_ids": list(result_ids)})
    decoded = [id_to_word[i] for i in result_ids]
    if trace:
        emit({"type": "decoded", "run": run, "ids": list(result_ids), "words": decoded})
    return decoded
...
if __name__ == "__main__":
    ...
    for run in range(3):
        seq = generate(["the", "cat"], trace=True, run=run)
