 ...
 */
export const OopCodeScene: React.FC<{
  title: string;
  code: string;
  previousCode?: string;
  schedule?: Anchor[];
  ...
  const traceLines = trace?.length ?? 0;
  const AVAIL = 724 - (hasTrace ? 60 + traceLines * 30 : 0);
  ...
  return (
    <AbsoluteFill style={{ background: c.bg }}>
      <GridBg opacity={0.06} color={mood ?? c.cool} />
      ...
      >
        ...
        >
          ...
          <div
            style={{
              ...surface(mood ?? c.cool, false),
              // mood rim: the colour arc (coral v1/v2 -> green v3/v4) has to be
              // visible on the element that is on screen 8 of 14 segments
              borderTop: `3px solid ${mood ?? c.cool}55`,
          ...
          >
            <div style={{ display: "flex", gap: 7, paddingLeft: 4, flexShrink: 0 }}>
              {[c.noise, c.accent, c.hit].map((dot) => (
                <span key={dot} style={{ width: 11, height: 11, borderRadius: radius.pill, background: `${dot}88`, boxShadow: `inset 0 1px 1px rgba(255,255,255,0.35), 0 0 6px ${dot}55` }} />
              ))}
              <span style={{ ...type.meta, color: c.muted, marginLeft: space.sm }}>notifications.py</span>
            </div>
            ...
            {trace && trace.length > 0 && frame >= (terminalFrames?.[0] ?? 0) - 20 && (
              <div
                style={{
                  flexShrink: 0,
